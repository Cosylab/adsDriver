...
std::map<long, int> ads_rc_to_epicsads_error_map = {
    ...
    {GLOBALERR_MISSING_ROUTE,
     EPICSADS_DISCONNECTED}, /* Occurs when ADS calls are made after AMS
                                connection is closed */
    {ADSERR_CLIENT_SYNCTIMEOUT,
     EPICSADS_DISCONNECTED}, /* Occurs when ADS device is disconnected, e.g.
                                cable is unplugged */
    {ADSERR_DEVICE_SYMBOLNOTFOUND,
     EPICSADS_NOT_RESOLVED} /* ADS symbol not found on ADS device */
};
...
std::map<long, std::string> errorMap = {
    ...
    {1282, "The mailbox has reached the maximum number of possible messages. "
           "The current sent message was rejected"},
    ...
    {4101, "No more free TCB (Task Control Block) available. Maximum number of "
           "TCBs is 64"},
    {4102,
    ...
    {10061, "A connection attempt failed because the connected party did not "
            "properly respond after a period of time, or established "
            "connection failed because connected host has failed to respond."},
    {10065, "No connection could be made because the target machine actively "
            "refused it"}};
