...
#define LOG_MSG_ASYN(asyn_user, log_level, log_level_str, format, ...)         \
    {                                                                          \
        ...
        asynPrint(asyn_user, log_level, "[%s] %s:%u %s(): [%s] " format "\n",  \
                  log_level_str, FILENAME, __LINE__, __func__,                 \
                  (port_name != nullptr ? port_name : "/"), ##__VA_ARGS__);    \
    }
...
#define LOG_WARN_ASYN(asyn_user, format, ...)                                  \
    LOG_MSG_ASYN(asyn_user, ASYN_TRACE_WARNING, "WARNING", format,             \
                 ##__VA_ARGS__)
...
#define LOG_MSG(log_level_str, format, ...)                                    \
    errlogPrintf("[%s] %s:%u %s(): " format "\n", log_level_str, FILENAME,     \
                 __LINE__, __func__, ##__VA_ARGS__)

#define LOG_ERR(format, ...) LOG_MSG("ERROR", format, ##__VA_ARGS__)

#define LOG_WARN(format, ...) LOG_MSG("WARNING", format, ##__VA_ARGS__)

#define LOG_TRACE(format, ...) LOG_MSG("TRACE", format, ##__VA_ARGS__)
